...
export default class SearchHistoryModel {
	//Do you want to cache per user, or across all users? All users to focus on common routes and dates
	static async saveSearch(user, searchParams) {
		const { origin, destination, departureDate, tripType, returnDate } =
			searchParams;
		const result = await pool.query(
			`INSERT INTO search_history(user_id, origin, destination, departure_date, return_date, trip_type )
			VALUES($1,$2,$3,$4,$5,$6)`,
			[user.id, origin, destination, departureDate, returnDate, tripType]
		);
	...
	static async checkSearchFrequency(searchParams) {
		let { origin, destination, departureDate, tripType, returnDate } =
			searchParams;
