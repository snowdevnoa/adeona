...
export default function TripModal({ modal, setModal, trip }) {
	...
	async function deleteTrip(e) {
		e.preventDefault();
		console.log("Are you sure you want to delete trip?");
	}
		...
		return (
			<main className="fixed w-screen h-screen bg-[rgb(0,0,0,0.5)] z-2 top-0 right-0">
				<div className="w-full h-full flex flex-col justify-center items-center">
					<form
						className="w-[80%] lg:w-[475px] text-[var(--adeona-blue-900)] bg-[var(--adeona-blue-300)] p-4 rounded-lg flex flex-col items-center space-y-3 relative"
						onSubmit={deleteTrip}
					>
						<h1 className="font-bold text-[2.25rem] text-center">{tripData.trip_name}</h1>
					...
					>
						...
						<div className="flex justify-center space-x-4">
							...
							<button
								...
								type="submit"
							>
								<Update className="mr-1 fill-inherit" /> Update
							</button>
						</div>
						{error && (
